...
public:
    ...
    {
        cv::Mat res;
        model->detect(image, res); // 调用 model->detect(image, res) 进行人检测。
...
*/
{
    ...
    for (int i = 0; i < faces.rows; ++i)
    {
        ...
        for (int j = 0; j < landmark_color.size(); ++j) // landmark_color.size() 等于5，表示有5个关键点
        {
            ...
            cv::circle(output_image, cv::Point(x, y), 2, landmark_color[j], 2);                                           // 在输出图像上绘制每个关键点。output_image是cv::Mat类型，表示输出图像。cv::Point(x, y)是关键点的坐标。2是关键点的半径，landmark_color[j]关键点的颜色，2是线条的粗细。
        }
    }
    return output_image;
}
    ...
    else // Call default camera
    {
        ...
        while (true)
        {
            ...
            if (key == 27) // ESC键退出
                ...
                effect_mode = 2;
            else if (key == 'a' || key == 'A') // A键减小
            {
                if (effect_mode == 0) // 高斯模糊模式
                {
                    blur_kernel_size = std::max(1, blur_kernel_size - 2);
                    cv::setTrackbarPos("Blur Kernel Size", "Parameters", blur_kernel_size);
                }
                else if (effect_mode == 1) // 像素化模式
                {
                    pixel_size = std::max(1, pixel_size - 1);
                    cv::setTrackbarPos("Pixel Size", "Parameters", pixel_size);
                }
            }
            else if (key == 'd' || key == 'D') // D键增大
            {
                if (effect_mode == 0) // 高斯模糊模式
                {
                    blur_kernel_size = std::min(max_kernel_size, blur_kernel_size + 2);
                    cv::setTrackbarPos("Blur Kernel Size", "Parameters", blur_kernel_size);
                }
                else if (effect_mode == 1) // 像素化模式
                {
